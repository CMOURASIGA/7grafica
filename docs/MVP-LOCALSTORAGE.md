...
- **Os "links seguros" e QR Codes públicos** (`/portal/orcamento/[token]` e
  `/portal/pedido/[token]`) só resolvem **no mesmo navegador/perfil** que
  gerou o orçamento ou pedido. Abrir o link em uma aba nova do mesmo
  navegador funciona (é o que os testes automatizados e a validação manual
  desta fase fazem); abrir em outro navegador, computador ou celular real
  mostra "link inválido ou expirado", porque os dados simplesmente não
  existem ali.
...
1. **Token público revogável e validado no servidor.** `crypto.randomUUID()`
   ...
   `/portal/orcamento/[token]` e `/portal/pedido/[token]` precisa ser
   validado por uma policy/RPC no backend, com possibilidade real de
   revogar um token (ex.: reenvio de orçamento, pedido cancelado) sem
